...
void OLED_ClearPoint(u8 x,u8 y)
{
	...
	m=y%8;
	n=1<<m;
	OLED_GRAM[x][i] &= ~n;  // 直接清除指定位, 替代原来三步取反操作
}
...
{
	u8 i,k,k1,k2;
	// 修复: u8 不可能 <0, 用 >=128/>=64 做边界检查; x2/y2 必须 >= x1/y1
	if((x1>=128)||(x2>=128)||(y1>=64)||(y2>=64)||(x1>x2)||(y1>y2))return;
	if(x1==x2)    //画竖线 (包含端点)
	{
		for(i=0;i<=(y2-y1);i++) OLED_DrawPoint(x1,y1+i);
	}
	else if(y1==y2)   //画横线 (包含端点)
	{
...
void OLED_DrawCircle(u8 x,u8 y,u8 r)
{
	...
	while(2 * b * b >= r * r)
	{
		// 修复: 添加边界检查, 防止坐标超出屏幕或 u8 下溢
		px = x + a; py = y - b;
		if(px >= 0 && px < 128 && py >= 0 && py < 64) OLED_DrawPoint(px, py);
		px = x - a; py = y - b;
		if(px >= 0 && px < 128 && py >= 0 && py < 64) OLED_DrawPoint(px, py);
		px = x - a; py = y + b;
		if(px >= 0 && px < 128 && py >= 0 && py < 64) OLED_DrawPoint(px, py);
